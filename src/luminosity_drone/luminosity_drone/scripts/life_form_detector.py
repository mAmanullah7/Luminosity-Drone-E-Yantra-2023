...
class swift():
    ...
    def __init__(self):
        ...
        # [x,y,z]
        # create a new variable to store the image
        self.image = None
    ...
    def image_callback(self, data):
        ...
        try:
            ...
            # organism_type = self.organism_type(num_leds)
            # self.publish_biolocation(organism_type,centroid_coordinates)
        ...
        # print(f"Centroid Coordinates: {centroid_coordinates}")

        if centroid_coordinates and len(centroid_coordinates) >= 2:
            biolocation_msg = Biolocation()
            biolocation_msg.organism_type = organism_type
            biolocation_msg.whycon_x, biolocation_msg.whycon_y = centroid_coordinates[:2]
            self.publish_biolocation(biolocation_msg)
    ...
    def pid(self):
        ...
        #------------------------------------------------------------------------------------------------------------       
            ...
            if self.current_setpoint_index == len(self.setpoint):
                # Disarm the drone when it reaches the final setpoint
                self.disarm()                
     #------------------------------------------------------------------------------------------------------------      
    ...
    def detect_oraganism(self, image):
        ...
        for i, c in enumerate(cnts):
            ...
            # Append centroid coordinates and area to the respective lists
                ...
                # rospy.loginfo(num_leds)
                areas.append(area)
        return len(cnts), centroid_coordinates
    ...
    def publish_biolocation(organism_type, centroid_coordinates):
        # Create Bioloction message
        biolocation_msg = Biolocation()
        biolocation_msg.organism_type = organism_type

        # Assuming centroid_coordinates is a tuple (x, y, z)
        biolocation_msg.whycon_x, biolocation_msg.whycon_y = centroid_coordinates[:2]
        biolocation_msg.organism_type = organism_type
        ...
        while not rospy.is_shutdown():
            if self.image is not None:
                ...
                if organism!= "unknown" :#and centroid_coordinates:
                    ...
                    # publish the Biolocation
                    self.publish_biolocation(organism ,centroid_coordinates)
                ...
                #   repeat the image after process
                self.image = None
